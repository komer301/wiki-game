...
(* let rec traverse () = match Queue.dequeue to_visit with | None -> () |
   Some current_node -> if not (Hash_set.mem visited current_node) then (
   Hash_set.add visited current_node; let adjacent_nodes = Map.find_exn
   network current_node in List.iter adjacent_nodes ~f:(fun next_node ->
   Queue.enqueue to_visit next_node)); traverse (); *)
...
let find_friend_group network ~(person : Person.t) : Person.t list =
  let not_graph =
    Set.fold
      network
  ...
  let rec traverse () =
    ...
    | None -> ()
    | Some person ->
      if not (Hash_set.mem visited person)
      then (
        Hash_set.add visited person;
        let adjacent_nodes = Map.find_exn not_graph person in
